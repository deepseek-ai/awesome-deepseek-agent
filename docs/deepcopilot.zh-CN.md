...
> **国内用户**：若 `api.deepseek.com` 连接不稳定，在 🔑 弹窗的 **Base URL** 字段填入 `https://api.deepseeki.com`。
...
从 [GitHub Releases](https://github.com/ZhouChaunge/DeepCopilot/releases) 下载最新 `.vsix`，然后执行：

```bash
code --install-extension deep-copilot-<version>.vsix
```

或在 VS Code 扩展面板右上角 `⋯` → **Install from VSIX…** 选择文件安装。

---

## 2. 配置 DeepSeek API Key

1. 点击活动栏中的 **🐋 Deep Copilot** 图标，打开聊天面板。
2. 点击面板**右下角** 🔑 按钮。
3. 粘贴你的 [DeepSeek API Key](https://platform.deepseek.com/api_keys)，点击**保存**。

> **国内用户提示**：若 `api.deepseek.com` 连接不稳定，可在同一弹窗的 **Base URL** 字段填入 `https://api.deepseeki.com`。

---

## 3. 选择 DeepSeek 模型

Deep Copilot 默认使用 `deepseek-v4-pro`。可在 **VS Code 设置**（`Ctrl/Cmd+,`）中切换模型：
...
| 模型 | 说明 |
|---|---|
| `deepseek-v4-pro` | 完整推理能力，支持最大思考强度，适合复杂编程任务。 |
| `deepseek-v4-flash` | 速度更快、成本更低，适合快速编辑与问答。 |

两款模型均支持最高 **100 万 token** 的上下文窗口。Deep Copilot 会在对话变长时自动压缩历史工具结果（可通过 `deepseekAgent.compactBudgetTokens` 配置，默认 `600000`）。

---

## 4. 首次运行

在侧边栏面板中直接输入需求，Deep Copilot 便会启动 Agent 循环：自主读写文件、搜索代码，并在你确认后执行 Shell 命令。

**示例提示词：**

```
给 src/utils.py 中所有函数添加类型注解
```

```
找出项目中所有 TODO 注释，并整理成 tasks.md
```
...
> 默认情况下，每次写文件或执行 Shell 命令都会弹窗请求你的确认。可通过设置调整审批策略（见第 5 节）。
...
// settings.json — 常用配置项
{
  ...
  // 审批模式："manual" | "auto-edit" | "autopilot" | "readonly"
  // - manual:     每次写文件或 Shell 命令都弹窗确认（默认，最安全）
  // - auto-edit:  写文件自动通过，Shell 仍需确认
  // - autopilot:  全部自动通过（仅适合受信任的工作区）
  // - readonly:   禁止所有写入与 Shell 操作
  "deepseekAgent.approvalMode": "manual",

  // 交互模式："agent"（工具调用）| "ask"（纯聊天）
  "deepseekAgent.interactionMode": "agent",
  ...
  // 自定义 API 入口（可选）
  "deepseekAgent.apiBaseUrl": "https://api.deepseek.com"
}
```

### 启用联网搜索（可选）

Deep Copilot 通过 [Tavily](https://app.tavily.com) 支持联网搜索。在同一个 **🔑** 弹窗中填入 Tavily API Key 即可启用。

### 接入 MCP 服务器（可选）

Deep Copilot 内置 MCP 客户端，可连接任意 MCP 兼容工具服务器：
...
`deepseek-v4-pro` 支持深度推理（Extended Thinking）。Deep Copilot 会自动启用该能力 —— 推理步骤以可展开的折叠块呈现在最终回答上方，无需额外配置，模型默认以最大推理强度运行。

如需使用更快的模型（不含推理）：

```jsonc
{
  "deepseekAgent.defaultModel": "deepseek-v4-flash"
}
```
...
| 功能 | 说明 |
|---|---|
| **Agent 循环** | 多轮工具调用，支持文件读写、Shell、搜索 |
| **Plan & Todos 面板** | Agent 自维护结构化任务列表，实时显示在侧边栏 |
| **一键回滚** | 撤销当前 Agent 轮次对所有文件的修改 |
| **用户记忆** | `~/.deepcopilot/memory.md` 自动注入每次系统提示词 |
| **编辑后 LSP 诊断** | 每次文件编辑后自动附加错误信息，模型可自我纠正 |
| **多会话并行** | 任务运行时可切换到新对话，回来后自动回放进度 |
| **MCP 客户端** | 可连接任意 MCP 兼容工具服务器 |
| **流式输出** | 逐 token 流式输出，含思维链展开与进度条 |
| **费用显示** | 底栏实时显示 token 数与人民币成本 |
...
Deep Copilot 在运行时按优先级顺序扫描三个目录来发现 Skill：

```
~/.deepcopilot/skills/   ← Deep Copilot 专属 Skill 目录（首次启动自动创建）
~/.claude/skills/        ← 兼容 Claude Code
~/.copilot/skills/       ← 兼容 GitHub Copilot
```

每个 Skill 是一个子目录，其中包含一个带 YAML frontmatter 头部的 `SKILL.md` 文件：

```markdown
---
name: my-skill
description: 显示在斜杠命令弹窗中的一行简介
argument-hint: 可选的提示文字
---

... Skill 指令内容 ...
```

Skill 按需加载，无需重启扩展。只需将新目录放入 `~/.deepcopilot/skills/`，下一条消息发出时即可生效。

**Skill 注入原理**

Skill 内容**不是**以用户消息的形式注入的。Deep Copilot 会在用户消息之前合成一对 `read_file` 工具调用 + 工具结果，并插入对话上下文：
...
这与 GitHub Copilot 内部注入 Skill 的方式完全一致。模型将 Skill 内容视为**自己读取**的信息，而非用户注入的文本——与系统提示注入或用户消息注入相比，指令遵循的可靠性更高。

三个目录之间采用「先匹配者优先」策略：`~/.deepcopilot/skills/` 中的同名 Skill 会静默覆盖 Claude Code 或 GitHub Copilot 目录中的对应项，方便你维护个人覆盖层而无需修改原始文件。

### 感知上下文缓存的系统提示

系统提示在 `__DYNAMIC_BOUNDARY__` 标记处一分为二：

- **静态部分**（标记之前）：行为原则、工具使用规则、语气风格——在所有请求中保持不变，可命中 DeepSeek 的上下文缓存，后续轮次只需支付缓存命中价格。
- **动态部分**（标记之后）：操作系统信息、工作区指令（`DEEPCOPILOT.md`）、用户记忆（`~/.deepcopilot/memory.md`）——每轮重新计算。

这一设计在最大化高价值静态内容缓存命中率的同时，保留了动态部分的个性化能力。
...
钩子的输出会被注入回模型上下文，Agent 可以直接读取测试结果并自我纠错——无需手动复制粘贴终端输出。

### 零后端架构

Deep Copilot 完全运行在 VS Code 扩展主机内。生产构建产物（`out/extension.js`）约 94 KB，无任何运行时 npm 依赖——仅依赖 VS Code API 与 Node.js 内置模块。VS Code 之外无任何需要安装、更新或保持运行的组件。
