...
在浏览器中使用 DeepSeek V4 模型：编辑文件、执行命令、浏览工作区、多 Agent 协同作业、挂载参考资料，对话中实时切换模型。
...
Agent 通过 OpenAI 兼容接口连接 DeepSeek，在 `config.ini` 中配置以下关键项：

| 配置项 | 值 | 说明 |
|--------|-----|------|
| 模型 | `deepseek-v4-pro`, `deepseek-v4-flash` | 界面中可实时切换 |
| API 地址 | `https://api.deepseek.com` | OpenAI 兼容接口 |
| 上下文窗口 | 1M tokens | 通过 `model_context_tokens_json` 设置 |
| 推理强度 | `max` / `high` | 在 `[misc]` 节设置 `reasoning_effort` |
| KV 缓存 | 默认启用 | 利用 DeepSeek prefix caching 降低成本 |

默认使用 `deepseek-v4-flash` 保障日常响应速度；需要深度推理时在界面切到 `deepseek-v4-pro`，无需重启。
...
Token 预估费用自动从 [DeepSeek 官方定价页](https://api-docs.deepseek.com/zh-cn/quick_start/pricing/) 获取并在界面中展示。Agent 启动时自动更新官方最新费率，仅作参考。
