...
DeepSeek V4 还支持**推理强度（reasoning effort）**控制。点击模型选择框旁的推理强度按钮（在**设置**中，或主输入框模型下拉旁的快速入口——后者只对当前会话生效），在 **关闭 / 低 / 中 / 高 / 最大** 中选择档位。复杂编码任务建议选择**最大**以开启深度思考。启用后 lite-code 会向 API 透传 `reasoning_effort` 参数。
