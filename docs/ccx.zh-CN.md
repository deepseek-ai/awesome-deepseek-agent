...
先在 Codex Responses 渠道页添加一个 Chat service type，例如打开 `http://localhost:3000/channels/responses`（如果你的 CCX 使用其他端口，如 `3688`，请替换端口）。这里配置的是 DeepSeek 的 OpenAI Chat 兼容上游；Base URL 是写入 CCX 的 DeepSeek 上游地址，不是 Codex 客户端连接 CCX 的本地地址：
...
保存后，**编辑**刚创建的 **DeepSeek Chat** 渠道，在编辑弹窗中打开 **规范化非常见 Chat role** 开关。该选项对应 CCX 的 `normalizeNonstandardChatRoles` 配置：当 Codex 的 Responses 请求被转换为 Chat Completions 后，`developer` 等 DeepSeek 不支持的 role 会在发往上游前规范化为 `user`；带 `tool_call_id` 的 `function` 消息会规范化为 `tool`。
...
为 Claude Code CLI 打开 Claude Messages 渠道页，例如 `http://localhost:3000/channels/messages`（如果你的 CCX 使用其他端口，如 `3688`，请替换端口），添加一个 Claude service type，并指向 DeepSeek 的 Anthropic 兼容端点：
...
Claude Code CLI 默认会以 Opus 4.7 这类 Claude 模型名发起请求。DeepSeek 官方 Anthropic 兼容接口可能会自动处理部分模型名；为了让路由结果更可控，推荐在 CCX 中显式配置模型重定向：
...
推荐为 Messages Claude service 配置 Model Mapping：

![Claude Code 渠道模型重定向](assets/ccx/channel-test-success-alt.png)

#### 3. 场景 A：Claude Code CLI

Claude Code CLI 使用 Messages API。使用上面配置的 Claude 渠道，填入 Anthropic 兼容的 DeepSeek Base URL、Key；模型映射为推荐配置。
...
Claude Code CLI 使用默认 Opus 4.7 发送 `/v1/messages` 请求。如果配置了模型重定向，CCX 会先按规则映射模型，再将请求路由至 DeepSeek 渠道；未配置时，请求模型名会交给 DeepSeek 官方接口处理。
