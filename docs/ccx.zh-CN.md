...
![DeepSeek 渠道模型重定向](../assets/ccx/redirect-editing-chat.png)
![Claude Code 渠道模型重定向](../assets/ccx/messages-channel-config.png)

#### 3. 场景 A：Claude Code CLI

Claude Code CLI 使用 Messages API。配置一个类型为 **Claude Code** 的渠道，填入 DeepSeek Base URL 和 Key，并设置模型映射：
...
![Responses → Chat 协议转换模型重定向](../assets/ccx/codex-responses-redirect.png)
...
渠道使用实时统计，可查看测试请求的响应延迟和吞吐：

![渠道实时统计](../assets/ccx/channel-test-success.png)
![渠道实时统计](../assets/ccx/channel-test-success-alt.png)
