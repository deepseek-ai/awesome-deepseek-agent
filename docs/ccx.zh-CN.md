...
配置完成后，可在「测试渠道」中验证连通性：

![测试成功](../assets/ccx/channel-test-success.png)

#### 3. 场景 A：Claude Code CLI

Claude Code CLI 使用 Messages API。配置一个类型为 **Claude Code** 的渠道，填入 DeepSeek Base URL 和 Key，并设置模型映射：

![Claude Messages 渠道配置](../assets/ccx/messages-channel-config.png)
...
Codex CLI 使用 OpenAI Responses API。配置一个类型为 **DeepSeek** 的渠道（OpenAI Chat），填入 DeepSeek Base URL、Key 和模型映射：

![DeepSeek Chat 渠道配置](../assets/ccx/chat-channel-config.png)
